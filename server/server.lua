...
AddEventHandler('Core:Shared:Ready', function()
    exports['mythic-base']:RequestDependencies('Dealerships', {
        ...
        RegisterVehicleSaleCallbacks()
            LoadDealershipShit()
        TriggerEvent('Dealerships:Server:RegisterCallbacks')
        ...
        Chat:RegisterAdminCommand("incstock", function(source, args, rawCommand)
            ...
            local isAdmin = player and player.Permissions:GetLevel() >= 3
            local hasPermission = dealership and Jobs.Permissions:HasPermissionInJob(source, dealership, 'dealership_manage')
            
            if not isAdmin and not hasPermission then
                Chat.Send.System:Single(source, "You don't have permission to use this command")
                return
            end
            ...
            help = "[Admin/Owner] Increase stock for specific vehicle at Dealership. Use \" for multiple words",
            params = {
                {
                    name = "Dealership ID",
...
function RegisterCallbacks()
    ...
    Callbacks:RegisterServerCallback('Dealerships:Sales:FetchData', function(source, dealerId, cb)
        if _dealerships[dealerId] and (Jobs.Permissions:HasPermissionInJob(source, dealerId, 'dealership_stock') or Jobs.Permissions:HasPermissionInJob(source, dealerId, 'dealership_sell')) then
            local dealerMData = Dealerships.Management:GetAllData(dealerId)
            if not dealerMData then
                dealerMData = _defaultDealershipSalesData
            end
            cb(true, Dealerships.Stock:FetchDealer(dealerId), os.time(), Loans:GetDefaultInterestRate(), dealerMData)
        else
            cb(false)
        end
    end)
    ...
    Callbacks:RegisterServerCallback('Dealerships:CheckPersonsCredit', function(source, data, cb)
        ...
        if not data or not data.dealerId or not data.SID then
            cb(false)
            return
        end

        local stateId = tonumber(data.SID)
        if not stateId or stateId <= 0 then
            cb(false)
            return
        end
        stateId = math.tointeger(stateId)

        if Jobs.Permissions:HasPermissionInJob(source, data.dealerId, 'dealership_sell') then
            local target = Fetch:SID(stateId)
            local firstName = 'Unknown'
            local lastName = 'Unknown'
            
            -- Try to get character name from online player first
            if target then
                local char = target:GetData('Character')
                if char then
                    firstName = char:GetData('First') or 'Unknown'
                    lastName = char:GetData('Last') or 'Unknown'
                end
            else
                -- Character is offline, query database
                local p = promise.new()
                Database.Game:findOne({
                    ...
                    if success and results and #results > 0 then
                        firstName = results[1].First or 'Unknown'
                        lastName = results[1].Last or 'Unknown'
                    end
                    p:resolve(true)
                end)
                Citizen.Await(p)
            ...
            if loanData and loanData.maxBorrowable and loanData.maxBorrowable > 0 then
                ...
                local loanLimit = (loanData.limit and loanData.limit > 0) and loanData.limit or 999999
                if not charVehicleLoans or #charVehicleLoans < loanLimit then
                    cb({
                        SID = stateId,
                        price = loanData.maxBorrowable,
                        score = loanData.creditScore or 0,
                ...
                cb({
                    SID = stateId,
                    price = false,
                    score = (loanData and loanData.creditScore) or 0,
                    name = string.format('%s %s', firstName, lastName)
                })
            end
        else
                ...
                if vehEnt.state.VIN and vehEnt.state.Owned and vehEnt.state.Make and vehEnt.state.Value and type(vehEnt.state.Value) == "number" and vehEnt.state.Value > 0 then
                    ...
                    if vehModel then
                        ...
                        if stockInfo and stockInfo.data and vehStrikes and not remainingLoan then
                            ...
                            local ownerData = vehicle and vehicle:GetData('Owner')
                            if ownerData and ownerData.Type == 0 then
                                local owner = Fetch:SID(ownerData.Id)
                                if owner then
