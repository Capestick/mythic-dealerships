...
-- Vehicle Definitions Management
DEALERSHIPS.VehicleDefinitions = {
    Fetch = function(self, dealership, vehicle)
        local p = promise.new()
        Database.Game:findOne({
    ...
    Add = function(self, dealership, vehicle, modelType, vehData)
        vehData = ValidateVehicleData(vehData)
        if _dealerships[dealership] and vehicle and vehData and modelType then
            local p = promise.new()
            local existing = Dealerships.VehicleDefinitions:Fetch(dealership, vehicle)
            if existing then
                ...
                Database.Game:updateOne({
                    ...
                    if success and result > 0 then
                        p:resolve({ success = true, existed = true })
                    else
                        p:resolve(false)
                    end
                end)
                ...
                Database.Game:insertOne({
                    ...
                    if success and inserted > 0 then
                        p:resolve({ success = true, existed = false })
                    else
                        p:resolve(false)
                    end
                end)
